...
public class PreBootReceiver extends BroadcastReceiver{
    private static final String TAG = "NamelessSettingsReceiver";
    private static final boolean LOCAL_LOGV = false;

    @Override
    public void onReceive(Context context, Intent intent) {
        if (LOCAL_LOGV) {
            Log.d(TAG, "Received pre boot intent. Attempting to migrate Nameless settings.");
        }

        ContentResolver contentResolver = context.getContentResolver();
        IContentProvider contentProvider = contentResolver.acquireProvider(
                NamelessSettings.AUTHORITY);

        try{
            contentProvider.call(contentResolver.getPackageName(),
                    NamelessSettings.CALL_METHOD_MIGRATE_SETTINGS, null, null);
