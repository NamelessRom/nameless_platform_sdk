...
package org.namelessrom.namelesssettings;
...
public class NamelessSettingsProvider extends ContentProvider {
    ...
    protected final SparseArray<NamelessDatabaseHelper> mDbHelpers = new SparseArray<NamelessDatabaseHelper>();
    ...
    static {
        sUriMatcher.addURI(NamelessSettings.AUTHORITY,
                NamelessDatabaseHelper.NamelessTableNames.TABLE_SYSTEM, SYSTEM);
        sUriMatcher.addURI(NamelessSettings.AUTHORITY,
                NamelessDatabaseHelper.NamelessTableNames.TABLE_SECURE, SECURE);
        sUriMatcher.addURI(NamelessSettings.AUTHORITY,
                NamelessDatabaseHelper.NamelessTableNames.TABLE_GLOBAL, GLOBAL);
        sUriMatcher.addURI(NamelessSettings.AUTHORITY,
                NamelessDatabaseHelper.NamelessTableNames.TABLE_SYSTEM + ITEM_MATCHER, SYSTEM_ITEM_NAME);
        sUriMatcher.addURI(NamelessSettings.AUTHORITY,
                NamelessDatabaseHelper.NamelessTableNames.TABLE_SECURE + ITEM_MATCHER, SECURE_ITEM_NAME);
        sUriMatcher.addURI(NamelessSettings.AUTHORITY,
                NamelessDatabaseHelper.NamelessTableNames.TABLE_GLOBAL + ITEM_MATCHER, GLOBAL_ITEM_NAME);
    }
    ...
    public boolean onCreate() {
        if (LOCAL_LOGV) Log.d(TAG, "Creating NamelessSettingsProvider");
    ...
    /**
     * Migrates Nameless settings for all existing users if this has not been run before.
     */
    private void migrateNamelessSettingsForExistingUsersIfNeeded() {
        boolean hasMigratedNamelessSettings = mSharedPrefs.getBoolean(PREF_HAS_MIGRATED_NAMELESS_SETTINGS,
                false);

        if (!hasMigratedNamelessSettings) {
            long startTime = System.currentTimeMillis();

            for (UserInfo user : mUserManager.getUsers()) {
                migrateNamelessSettingsForUser(user.id);
            }

            mSharedPrefs.edit().putBoolean(PREF_HAS_MIGRATED_NAMELESS_SETTINGS, true).commit();
    ...
    /**
     * Migrates Nameless settings for a specific user.
     * @param userId The id of the user to run Nameless settings migration for.
     */
    private void migrateNamelessSettingsForUser(int userId) {
        synchronized (this) {
            if (LOCAL_LOGV) Log.d(TAG, "Nameless settings will be migrated for user id: " + userId);

            // Migrate system settings
            int rowsMigrated = migrateNamelessSettingsForTable(userId,
                    NamelessDatabaseHelper.NamelessTableNames.TABLE_SYSTEM,
                    NamelessSettings.System.LEGACY_SYSTEM_SETTINGS);
            if (LOCAL_LOGV) Log.d(TAG, "Migrated " + rowsMigrated + " to Nameless system table");

            // Migrate secure settings
            rowsMigrated = migrateNamelessSettingsForTable(userId,
                    NamelessDatabaseHelper.NamelessTableNames.TABLE_SECURE,
                    NamelessSettings.Secure.LEGACY_SECURE_SETTINGS);
            if (LOCAL_LOGV) Log.d(TAG, "Migrated " + rowsMigrated + " to Nameless secure table");

            // Migrate global settings
            rowsMigrated = migrateNamelessSettingsForTable(userId,
                    NamelessDatabaseHelper.NamelessTableNames.TABLE_GLOBAL,
                    NamelessSettings.Global.LEGACY_GLOBAL_SETTINGS);
            if (LOCAL_LOGV) Log.d(TAG, "Migrated " + rowsMigrated + " to Nameless global table");
        }
    }

    /**
     * Migrates Nameless settings for a specific table and user id.
     * @param userId The id of the user to run Nameless settings migration for.
     * @param tableName The name of the table to run Nameless settings migration on.
     * @param settings An array of keys to migrate from {@link Settings} to {@link NamelessSettings}
     * @return Number of rows migrated.
     */
    private int migrateNamelessSettingsForTable(int userId, String tableName, String[] settings) {
        ...
        for (String settingsKey : settings) {
            ...
            if (tableName.equals(NamelessDatabaseHelper.NamelessTableNames.TABLE_SYSTEM)) {
                settingsValue = Settings.System.getStringForUser(contentResolver, settingsKey,
                        userId);
            } else if (tableName.equals(NamelessDatabaseHelper.NamelessTableNames.TABLE_SECURE)) {
                settingsValue = Settings.Secure.getStringForUser(contentResolver, settingsKey,
                        userId);
            } else if (tableName.equals(NamelessDatabaseHelper.NamelessTableNames.TABLE_GLOBAL)) {
    ...
    public Bundle call(String method, String request, Bundle args) {
        ...
        if (args != null) {
            int reqUser = args.getInt(NamelessSettings.CALL_METHOD_USER_KEY, callingUserId);
        ...
        // Migrate methods
        if (NamelessSettings.CALL_METHOD_MIGRATE_SETTINGS.equals(method)) {
            migrateNamelessSettingsForExistingUsersIfNeeded();

            return null;
        } else if (NamelessSettings.CALL_METHOD_MIGRATE_SETTINGS_FOR_USER.equals(method)) {
            migrateNamelessSettingsForUser(callingUserId);

            return null;
        }

        // Get methods
        if (NamelessSettings.CALL_METHOD_GET_SYSTEM.equals(method)) {
            return lookupSingleValue(callingUserId, NamelessSettings.System.CONTENT_URI, request);
        } else if (NamelessSettings.CALL_METHOD_GET_SECURE.equals(method)) {
            return lookupSingleValue(callingUserId, NamelessSettings.Secure.CONTENT_URI, request);
        } else if (NamelessSettings.CALL_METHOD_GET_GLOBAL.equals(method)) {
            return lookupSingleValue(callingUserId, NamelessSettings.Global.CONTENT_URI, request);
        }
        ...
        if (NamelessSettings.CALL_METHOD_PUT_SYSTEM.equals(method)) {
            insertForUser(callingUserId, NamelessSettings.System.CONTENT_URI, values);
        } else if (NamelessSettings.CALL_METHOD_PUT_SECURE.equals(method)) {
            insertForUser(callingUserId, NamelessSettings.Secure.CONTENT_URI, values);
        } else if (NamelessSettings.CALL_METHOD_PUT_GLOBAL.equals(method)) {
            insertForUser(callingUserId, NamelessSettings.Global.CONTENT_URI, values);
        }
    ...
    private Cursor queryForUser(int userId, Uri uri, String[] projection, String selection,
        ...
        NamelessDatabaseHelper dbHelper = getOrEstablishDatabase(getUserIdForTable(tableName, userId));
    ...
    private Uri insertForUser(int userId, Uri uri, ContentValues values) {
        ...
        if (NamelessDatabaseHelper.NamelessTableNames.TABLE_SYSTEM.equals(tableName)) {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        if (!TextUtils.isEmpty(selection) && selectionArgs.length > 0) {
            ...
            int callingUserId = UserHandle.getCallingUserId();
            NamelessDatabaseHelper dbHelper = getOrEstablishDatabase(getUserIdForTable(tableName,
                    callingUserId));
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        // NOTE: update() is never called by the front-end NamelessSettings API, and updates that
    ...
    /**
     * Tries to get a {@link NamelessDatabaseHelper} for the specified user and if it does not exist, a
     * new instance of {@link NamelessDatabaseHelper} is created for the specified user and returned.
     * @param callingUser
     * @return
     */
    private NamelessDatabaseHelper getOrEstablishDatabase(int callingUser) {
        ...
        try {
            NamelessDatabaseHelper dbHelper;
            synchronized (this) {
    ...
    /**
     * Check if a {@link NamelessDatabaseHelper} exists for a user and if it doesn't, a new helper is
    ...
    private void establishDbTracking(int userId) {
        ...
        synchronized (this) {
            ...
            if (dbHelper == null) {
                ...
                dbHelper = new NamelessDatabaseHelper(getContext(), userId);
    ...
    private String getTableNameFromUriMatchCode(int code) {
        switch (code) {
            case SYSTEM:
            case SYSTEM_ITEM_NAME:
                return NamelessDatabaseHelper.NamelessTableNames.TABLE_SYSTEM;
            case SECURE:
            case SECURE_ITEM_NAME:
                return NamelessDatabaseHelper.NamelessTableNames.TABLE_SECURE;
            case GLOBAL:
            case GLOBAL_ITEM_NAME:
                return NamelessDatabaseHelper.NamelessTableNames.TABLE_GLOBAL;
            default:
                throw new IllegalArgumentException("Invalid uri match code: " + code);
        }
    }
    ...
    private int getUserIdForTable(String tableName, int userId) {
        return NamelessDatabaseHelper.NamelessTableNames.TABLE_GLOBAL.equals(tableName) ?
                UserHandle.USER_OWNER : userId;
    }

    /**
     * Modify setting version for an updated table before notifying of change. The
     * {@link NamelessSettings} class uses these to provide client-side caches.
     * @param uri to send notifications for
     * @param userId
     */
    private void notifyChange(Uri uri, String tableName, int userId) {
        String property = null;
        final boolean isGlobal = tableName.equals(NamelessDatabaseHelper.NamelessTableNames.TABLE_GLOBAL);
        if (tableName.equals(NamelessDatabaseHelper.NamelessTableNames.TABLE_SYSTEM)) {
            property = NamelessSettings.System.SYS_PROP_NAMELESS_SETTING_VERSION;
        } else if (tableName.equals(NamelessDatabaseHelper.NamelessTableNames.TABLE_SECURE)) {
            property = NamelessSettings.Secure.SYS_PROP_NAMELESS_SETTING_VERSION;
        } else if (isGlobal) {
            property = NamelessSettings.Global.SYS_PROP_NAMELESS_SETTING_VERSION;
        }
    ...
    private void validateSystemSettingNameValue(String name, String value) {
        NamelessSettings.System.Validator validator = NamelessSettings.System.VALIDATORS.get(name);
        if (validator == null) {
