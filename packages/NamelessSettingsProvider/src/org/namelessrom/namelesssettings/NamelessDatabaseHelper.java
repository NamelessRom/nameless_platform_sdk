...
/**
 * The NamelessDatabaseHelper allows creation of a database to store Nameless specific settings
 * for a user in System, Secure, and Global tables.
 */
public class NamelessDatabaseHelper extends SQLiteOpenHelper{
    private static final String TAG = "NamelessDatabaseHelper";
    private static final boolean LOCAL_LOGV = false;

    private static final String DATABASE_NAME = "namelesssettings.db";
    private static final int DATABASE_VERSION = 2;

    static class NamelessTableNames {
    ...
    /**
     * Creates an instance of {@link NamelessDatabaseHelper}
     * @param context
     * @param userId
     */
    public NamelessDatabaseHelper(Context context, int userId) {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        try {
            createDbTable(db, NamelessTableNames.TABLE_SYSTEM);
            createDbTable(db, NamelessTableNames.TABLE_SECURE);

            if (mUserHandle == UserHandle.USER_OWNER) {
                createDbTable(db, NamelessTableNames.TABLE_GLOBAL);
            }
            ...
            if (LOCAL_LOGV) {
                Log.d(TAG, "Successfully created tables for nameless settings db");
            }
        } finally {
            db.endTransaction();
        }
        ...
        if (upgradeVersion < newVersion) {
            Log.w(TAG, "Got stuck trying to upgrade db. Old version: " + oldVersion
                    + ", version stuck at: " +  upgradeVersion + ", new version: "
                            + newVersion + ". Must wipe the nameless settings provider.");

            dropDbTable(db, NamelessTableNames.TABLE_SYSTEM);
            dropDbTable(db, NamelessTableNames.TABLE_SECURE);

            if (mUserHandle == UserHandle.USER_OWNER) {
                dropDbTable(db, NamelessTableNames.TABLE_GLOBAL);
            }
